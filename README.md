...
[![Demo 视频封面](demo/demo_poster.png)](demo/demo.mp4)

> ▶️ 点击封面观看完整演示（约 1 分 33 秒）：**自定义题目输入 → Hy3 分步解答 → 答案判定 → 过程审计 → 首错步定位与错误类型归因**，完整覆盖「过程正确性判定、错误步骤定位、CBU 识别」三大核心能力。演示用题见 [`demo/DEMO_PROBLEMS.md`](demo/DEMO_PROBLEMS.md)。
...
- **GPU**：2 × NVIDIA H200
- **模型部署**：受显存与部署成本限制，本项目采用 **GPTQ-int4 量化版** Hy3（`hy3-gptq-int4`）而非全精度版本——双卡 H200 下 int4 量化即可流畅推理，且对数学推理能力影响可控；评测结论均基于该量化版本得出。
- **推理框架**：vLLM 0.22.0（OpenAI 兼容 API 模式）
...
<details>
<summary>⚖️ LLM-as-Judge 提示词全文</summary>


完整 prompt（含解题 prompt、反向验证续写 prompt）见 [PROJECT_REPORT.md 附录 A](PROJECT_REPORT.md#附录-a核心-prompt-全文)。以下为两套 judge system prompt 全文：

**普通审查 prompt（L1–L3 题使用，`evaluator/llm_judge.py`）：**

```text
你是一位严格的数学解题过程审查专家。你的任务是对比题目和解题步骤，判断每一步是否存在逻辑或概念错误，并给出最贴切的错误类型。
...
**Research 严格审查 prompt（L4 研究级题使用）：**

```text
你是一位极其严格的数学研究论文审稿人，正在审查一道研究级数学题的解题过程。该题可能涉及高等数论、代数几何、分析等前沿领域。
...
> 💡 审查原则第 6 条与 few-shot 判例是迭代 4 的校准改动：修正前裁判把近半错误笼统归入「跳步推导」，加入具体类型优先匹配规则后错误类型分布明显均衡。
